...
public static class KeyboardAction
{
    ...
    public static Task PressKeyAsync(Key key, KeyModifiers modifiers = default, CancellationToken ct = default)
        => HotkeyAsync(modifiers, [key], ct);

    // Presses modifiers, then every key in keys down together as a true simultaneous chord, then releases all in reverse order.
    public static Task HotkeyAsync(KeyModifiers modifiers, IReadOnlyList<Key> keys, CancellationToken ct = default) => Task.Run(() =>
    {
    ...
    internal static void KeyDownNow(Key key) => NativeMethods.SendInputs(KeyInput(key, isKeyUp: false));

    // Releases a previously held key.
    public static Task KeyUpAsync(Key key, CancellationToken ct = default) =>
        Task.Run(() => NativeMethods.SendInputs(KeyInput(key, isKeyUp: true)), ct);
    ...
    internal static void ReleaseKeysNow(IReadOnlyList<Key> keys)
    {
